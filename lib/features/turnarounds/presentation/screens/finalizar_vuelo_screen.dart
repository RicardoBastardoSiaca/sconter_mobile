...
class FinalizarVueloScreen extends StatelessWidget {
  const FinalizarVueloScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Finalizar Vuelo')),
      body: const _FinalizarVueloView(),
    );
  }
}

class _FinalizarVueloView extends ConsumerStatefulWidget {
  const _FinalizarVueloView();

  @override
  ConsumerState<ConsumerStatefulWidget> createState() {
    return _FinalizarVueloViewState();
  }

  // @override
  // State<_FinalizarVueloView> createState() => _FinalizarVueloViewState();
}

class _FinalizarVueloViewState extends ConsumerState<_FinalizarVueloView> {
  final introKey = GlobalKey<IntroductionScreenState>();

  Future<bool> _onIntroEnd(BuildContext context) async {
    print("onIntroEnd");

    final bool? result = await showConfirmationDialogNoClose2(
      context: context,
      title: 'Finalizar Vuelo',
      message: 'Esta seguro de finalizar el vuelo?',
    ...
    if (result == true) {
      ...
      final response = ref.read(selectedTurnaroundProvider)!.fkVuelo.tipoServicio != null
          ? await ref
              .read(turnaroundProvider.notifier)
              .finalizarVuelo(body)
          : await ref
            .read(turnaroundProvider.notifier)
            // .finalizarVueloServicioMiscelaneo(body);
            .finalizarVueloServicioMiscelaneo(body['id_vuelo'] as int);
    ...
    return SafeArea(
      child: IntroductionScreen(
        ...
        pages: [
          ...
          // No mostrar para servicios miscelaneos
          if (turnaround.fkVuelo.tipoServicio != null)
          PageViewModel(
